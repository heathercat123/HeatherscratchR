<?php
    ...
    $is_comment_reply_enabled = ($isLogged && $comment_level <= 1);
?>
...
$is_comment_ignored = in_array($comment['Gcomment']['id'], $ignored_comments);

$is_comment_ok = (!$is_commenter_ignored && !$is_comment_ignored && !$is_deleted_commenter_ignored);
$markcomment_url = '/galleries/markcomment/' . $theme_id . '/' . $comment_id . '/0';
$can_delete_comment = true;
if(($isThemeOwner ||isset($users_permission['delete_gallery_comments'])) && !$isAdmin && $comment['User']['role'] == 'admin'){
...
<div class="comment" id="<?= $comment_id ?>">
    ...
    <h5>
        ...
        <?php if (empty($date) || (isset($date) && $date != "Now")):
            printf(___('%s ago', true), friendlyDate($comment['Gcomment']['timestamp']));
        endif; ?>
    </h5>
    ...
    <?php if ($is_comment_ok):
        ...
        if ($comment['Gcomment']['replies'] > NUM_COMMENT_REPLY) {
            $comment_links[] = sprintf($more_reply_link, $comment_id,
                    $ajax->link(___('view all replies', true), '#',
                        array(  'url'       => '/galleries/display_replies/' . $comment_id . '/0',
        ...
        if ($isLogged && $can_delete_comment &&($isAdmin || $isThemeOwner
